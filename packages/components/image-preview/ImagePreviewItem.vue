...
function checkClose(event: TouchEvent) {
...
<template>
  <SwipeItem ref="swipeItem" :class="bem('swipe-item')" @touchstart-passive="onTouchStart" @touchend="onTouchEnd" @touchcancel="onTouchEnd">
    <template v-if="slots.image">
      <view :class="bem('image-wrap')">
        <slot name="image" />
      </view>
      <Image
        ref="imageRef" :src="props.src"
        fit="contain"
        :class="bem('image', { vertical })"
        :style="imageStyle"
        @load="onLoad"
      >
        <template #loading>
